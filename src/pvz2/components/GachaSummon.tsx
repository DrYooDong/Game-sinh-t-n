...
export const GachaSummon: React.FC<GachaSummonModalProps> = ({ player, onSummon }) => {
  ...
  // Gacha Pull Mechanics with Native C++ Wasm Bridge Roll
  const performSummon = (count: number) => {
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < count; i++) {
        // Native C++ WebAssembly Algorithm Roll
        const rolledRarity = wasmBridge.pvz2RollGachaRarity(player.gachaPityCount + i);
        const matchingCards = ALL_CARDS.filter(c => c.rarity === rolledRarity);
        const pool = matchingCards.length > 0 ? matchingCards : ALL_CARDS;
