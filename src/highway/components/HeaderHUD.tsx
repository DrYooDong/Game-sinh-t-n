    ...
    <header className="bg-[#0c0c0e] border-b border-[#2d2d30] px-3 sm:px-5 py-2 flex flex-col gap-2 text-[#e0e0e0] z-30 select-none shadow-2xl relative">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 flex-wrap font-mono">
          ...
          {/* Portal / Return to Multiverse Select */}
          {onReturnToWorldSelect && (
            <button
              onClick={() => {
                soundEngine.playClick();
              ...
              className="px-2.5 py-1.5 bg-cyan-950/80 hover:bg-cyan-900 border border-cyan-500/50 hover:border-cyan-400 text-cyan-300 text-xs font-bold rounded flex items-center gap-1.5 transition shadow-[0_0_10px_rgba(6,182,212,0.2)]"
              title="Quay lại sảnh Chọn Thế Giới / Multiverse Portal"
            >
              <Globe className="w-3.5 h-3.5 text-cyan-400" />
              <span className="hidden sm:inline">CHỌN THẾ GIỚI</span>
            </button>
          )}
        </div>
      </div>
