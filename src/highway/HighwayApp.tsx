...
export interface HighwayAppProps {
  onReturnToWorldSelect?: () => void;
}

export function HighwayApp({ onReturnToWorldSelect }: HighwayAppProps = {}) {
    ...
    <div className="flex flex-col h-screen w-screen bg-[#050506] text-white overflow-hidden select-none font-mono">
      ...
      <VehicleCustomizerModal
        ...
        onUpgradePart={(partName, targetLevel) => {
          setVehicleStats((prev) => ({ ...prev, [partName]: targetLevel }));
        }}
        onUpgradeCore={handleUpgradeVehicleCore}
        onToggleAC={handleToggleAC}
        onSetWaterTemp={(temp) => {
          setVehicleStats((prev) => ({ ...prev, waterTankTemp: temp }));
        }}
      ...
      <InventoryModal
        ...
        petStats={petStats}
        onEquipWeapon={(wId) => {
          setPlayerStats((prev) => ({ ...prev, equippedWeaponId: wId }));
        }}
        ...
        onHatchPetEgg={() => {
          setPetStats((prev) => ({ ...prev, unlocked: true }));
        }}
        onLearnBlueprint={handleLearnBlueprint}
      />
      ...
      <WorldChatAndTradeModal
        ...
        onBuyMarketItem={(listingId) => {
          ...
          const reqItem = inventory.find(
            (i) =>
              (l.requestedItemId && i.id === l.requestedItemId) ||
              (l.requestedItemName && i.name.toLowerCase().includes(l.requestedItemName.toLowerCase())) ||
              (l.requestedItemId && i.name.toLowerCase().includes(l.requestedItemId.toLowerCase())) ||
              (l.requestedItemId && i.id.includes(l.requestedItemId))
          );
          const reqQty = l.requestedQuantity || 1;
          if (!reqItem || reqItem.quantity < reqQty) {
            alert(`Bạn không có đủ ${reqQty}x ${l.requestedItemName || l.requestedItemId} để mua!`);
            return;
          }
          ...
          setInventory((inv) => {
            let next = inv
              .map((i) => {
                if (i.id === reqItem.id) {
                  return { ...i, quantity: i.quantity - reqQty };
                }
                return i;
              })
              .filter((i) => i.quantity > 0);
            ...
            if (existIdx >= 0) {
              ...
              next.push({
                id: `mkt_${Date.now()}`,
                name: l.offeredItemName || l.offeredItemId,
                category: isBp ? 'blueprint' : 'material',
        ...
        onCreateMarketListing={(offeredItemId, offeredQty, reqItemId, reqQty) => {
          soundEngine.playClick();
          // Deduct offered item from inventory
          setInventory((inv) =>
            inv
          ...
          setMarketListings((prev) => [
            {
              ...
              offeredItemName: offeredItem?.name || offeredItemId,
              offeredQuantity: offeredQty,
              offeredItemRarity: offeredItem?.rarity || 'common',
              requestedItemId: reqItemId,
              requestedItemName: reqItemId,
          ...
          setTimeout(() => {
            ...
            setChatHistory((prev) => [
              ...prev,
              {
                ...
                isPlayer: false,
                isSystem: true,
                content: `[GIAO DỊCH THÀNH CÔNG] Vật phẩm của bạn đã được bán với giá ${reqQty}x ${reqItemId}!`,
                timestamp: `${Math.floor((gameTimeMinutes % 1440) / 60)
                  .toString()
                  .padStart(2, '0')}:${(gameTimeMinutes % 60).toString().padStart(2, '0')}`,
                channel: 'system',
      ...
      <QuestAchievementModal
        ...
        onClaimQuestReward={(qId) => {
          ...
          if (q.rewards.badges) {
            setPlayerStats((p) => ({
              ...p,
              courageBadges: p.courageBadges + (q.rewards.badges || 0),
            }));
          }
        ...
        onClaimAchievementReward={(aId) => {
          ...
          soundEngine.playCritFanfare();
          setAchievements((achs) => achs.map((a) => (a.id === aId ? { ...a, unlocked: true } : a)));
      ...
      <SupplyStationModal
        ...
        courageBadges={playerStats.courageBadges}
        onBuySupply={(itemId, qty, pricePerUnit) => {
          const totalCost = qty * pricePerUnit;
          if (playerStats.courageBadges < totalCost) return;
          soundEngine.playClick();
          setPlayerStats((p) => ({ ...p, courageBadges: p.courageBadges - totalCost }));
          setInventory((inv) => {
      ...
      <SurvivalSkillsModal
        ...
        playerStats={playerStats}
        onUpgradeSkill={(skillId, cost) => handleUnlockSkill(skillId)}
        onResetSkills={() => {
          setPlayerStats((p) => ({
            ...p,
            unlockedSkills: {},
